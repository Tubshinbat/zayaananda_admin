...
const Faqs = (props) => {
  ...
  const handleEdit = () => {
    if (selectedRowKeys.length != 1) {
      ...
      history.push(`/booking/edit/${selectedRowKeys[0]}`);
    }
  };

  const handleDelete = () => {
    props.deleteMultBooking(selectedRowKeys);
  };
  ...
  const exportExcel = async () => {
    const query = queryBuild();
    const response = await axios.get("bookings/excel?" + query);
    let excelData = [];
    if (response) {
          ...
          <div className="container-fluid">
            <div className="card datatable-card">
              <div className="card-header">
                <h3 className="card-title">Цаг авалт</h3>
              </div>
...
const mapStateToProps = (state) => {
  return {
    ...
    pagination: state.bookingReducer.paginationLast,
    excelData: state.bookingReducer.excelData,
  };
};

const mapDispatchToProp = (dispatch) => {
  return {
    loadBooking: (query) => dispatch(actions.loadBooking(query)),
